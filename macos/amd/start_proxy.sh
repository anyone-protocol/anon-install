...
if [[ "$(sw_vers --productName)" == "macOS" ]];then
	...
	if [[ ! -f anon-live-macos-arm64.zip ]];then
		curl -m 30 -fsSLO https://github.com/anyone-protocol/ator-protocol/releases/latest/download/anon-live-macos-arm64.zip
	fi
	...
	unzip -o anon-live-macos-arm64.zip anon > /dev/null 2>&1	
	kill $(pgrep anon) > /dev/null 2>&1	
